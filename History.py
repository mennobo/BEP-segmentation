...
#Read out JSON files mnt-by: serverius-mnt
...
print(unmerged_total - merged_total)
print(len(raw_net_list))
print(len(merged_net_list))
f'merged total: {merged_total}, unmerged_total: {unmerged_total}, difference: {unmerged_total - merged_total}'
with open('serverius_netblocks_merged.txt', 'w+') as file:
    for i in merged_net_list:
        file.write(str(i) + ', ')
with open('serverius_netblocks_unmerged.txt', 'w+') as file:
    for i in raw_net_list:
        file.write(str(i) + ', ')

#Read out domain information
line1_pdns = pd.read_csv('Serverius_line_1.csv')
line1_pdns = line1_pdns['result'].str.split("\t", n = 5, expand = True)
line1_pdns.columns = ['domain', 'type', 'ip', 'count', 'fromtime', 'totime']
line23_pdns = pd.read_csv('Serverius_line_2_3.csv')
line23_pdns = line23_pdns['result'].str.split("\t", n = 5, expand = True)
line23_pdns.columns = ['domain', 'type', 'ip', 'count', 'fromtime', 'totime']
line456_pdns = pd.read_csv('Serverius_line_4_5_6.csv')
line456_pdns = line456_pdns['result'].str.split("\t", n = 5, expand = True)
line456_pdns.columns = ['domain', 'type', 'ip', 'count', 'fromtime', 'totime']
_PDNS = pd.concat([line1_pdns, line23_pdns, line456_pdns], axis = 0)
...
_PDNS.tail()
_PDNS.to_csv('serverius_pdns.csv')
_ISP_INFO.to_csv('maxmind_isp.csv')
...
def change_type(element):
    if element in cont_domains:
        ...
        return 'A'
pdns_2018_9_10.type = pdns_2018_9_10.domain.apply(lambda x: change_type(x))

#Load maxmind db information
_ISP_INFO = pd.read_csv('maxmind_geoip/GeoIP2-ISP-Blocks-IPv4.csv')
_ANONYMOUS_IP = maxminddb.open_database('maxmind_geoip/GeoIP2-Anonymous-IP.mmdb')
_ISP_INFO.columns = ['network', 'isp', 'org', 'asn', 'as_org']
_ISP_INFO['network'] = _ISP_INFO['network'].apply(lambda x: IPNetwork(x))
_ISP_INFO.tail()

#Add domains to ip-addresses
#takes about 36 minutes for all entries
index_pdns = 0
index_isp = 0
start = time.time()

while index_pdns <= len(_PDNS):
    ip_to_match = IPAddress(_PDNS.iloc[index_pdns, 2])
    net_to_match = IPNetwork(_ISP_INFO.iloc[index_isp, 0])
#     print(('ip: {0}, network: {1}').format(ip_to_match, net_to_match))
    if ip_to_match > net_to_match[-1]:
        index_isp += 1
        continue
    elif ip_to_match in net_to_match: #match on network
        isp = _ISP_INFO.iloc[index_isp, 1]
        if index_pdns % 5000 == 0:
            print(('match! ip: {0}, net {1}, isp: {2}').format(ip_to_match, net_to_match, isp))
            print(('index: {0}, time taken: {1}').format(index_pdns, time.time() - start))
        _PDNS.iloc[index_pdns, 6] = isp #find isp and add it to _PDNS
        index_pdns += 1
        continue
    else:
        index_pdns += 1

#get ttl information with ns and domain information
...
def get_ttl(row):
    ...
    if ans:
        ...
        return np.nan
pdns_2018_9_10.ttl = pdns_2018_9_10.apply(get_ttl, axis=1)
...
#write nsrecords to file
with open('./nsrecords_serverius.txt', 'w+') as file:
    for k, v in gzip_dic1.items():
        line = k + '|'
        for ns in v:
            line += ns + ' '
        line += '\n'
...
def check_dom(domain):
    if domain in gzip_dic1:
        ...
        return np.nan
pdns_2018_9_10['ns_ass'] = pdns_2018_9_10['domain'].apply(check_dom)
pdns_2018_9_10
...
def get_ns(row):
    ...
    if ans != '':
        ...
        return np.nan
pdns_2018_9_10.ns_ass = pdns_2018_9_10.apply(get_ns, axis=1)

#add dhe information (verifying web services)
import lz4.frame
serverius_set = set()
...
printlist = '5.178.64.0/21, 5.255.64.0/19, 46.249.32.0/19, 89.47.1.0/24, 91.142.132.0/24, 91.198.106.0/24, 91.205.192.0/23, 91.216.34.0/24,  91.220.37.0/24, 91.220.53.0/24, 91.221.69.0/24, 93.158.200.0/21, 93.158.208.0/20, 178.21.16.0/21, 185.8.176.0/22, 185.12.12.0/22, 185.53.160.0/22, 185.79.112.0/22, 185.116.166.0/23, 193.23.143.0/24, 194.247.30.0/23, 194.247.38.0/24'
printlist = printlist.split(', ')
# result = pd.DataFrame(index = ['average age'], columns = ['A'])
age = []
for item in net_dic:
    block = net_dic[item]
    test = block['age'].mean()
